...
public class Leoben3 : Drzwi
{
    ...
    RóżnicaProstokątów InoxMałyGeometriaStronaPrzylgowa { get; set; }
    ...
    RóżnicaProstokątów InoxMałyGeometriaStronaFelcowa { get; set; }

    Tekstura InoxTekstura { get; set; }
    Tekstura InoxMałyTekstura { get; set; }
    Prostokąt SzkłoGeometria { get; set; }
    Tekstura SzkłoTekstura { get; set; }

    public Leoben3(Ościeżnica ościeżnica, Skrzydło skrzydło, SzyldDolny szyldDolny, Klamka klamka, SzyldGórny szyldGórny,
    ...
    {
        OdległośćPomiędzyInox = 15;
        OdsunięcieOdOsi = 127.5d;
        ParametrOtworu = 540;
        ...
        //Duży inox przylga
        InoxZewSzerokośćStronaPrzylgowa = Skrzydło.SzerokośćWPrzyldze - ParametrOtworu + (Skrzydło.SzerokośćWPrzyldze - OtwórSzerokość) / 2 + 20 - OdsunięcieOdOsi - OdległośćInoxOdKrawędziStronaPrzylgowa;
        ...
        //Mały inox przylga
        InoxMałyZewSzerokośćStronaPrzylgowa = InoxZewWysokośćStronaPrzylgowa;
        InoxMałyZewWysokośćStronaPrzylgowa = InoxZewWysokośćStronaPrzylgowa;
        ...
        //Duży inox felc
        InoxZewSzerokośćStronaFelcowa = Skrzydło.SzerokośćWPrzyldze - ParametrOtworu + (Skrzydło.SzerokośćWPrzyldze - OtwórSzerokość) / 2 + 20 - OdsunięcieOdOsi - OdległośćInoxOdKrawędziStronaFelcowa - PrzylgaBoczna;
        ...
        InoxMałyGeometriaStronaPrzylgowa = new(new Prostokąt(InoxMałyZewSzerokośćStronaPrzylgowa, InoxMałyZewWysokośćStronaPrzylgowa, PraweIsTrue, InoxMałyZewZaokrąglenieStronaPrzylgowa), new Prostokąt(InoxMałyWewSzerokośćStronaPrzylgowa, InoxMałyWewWysokośćStronaPrzylgowa, PraweIsTrue, InoxMałyWewZaokrąglenieStronaPrzylgowa, OdsunięcieInoxMałyWewWzględemOsiXStronaPrzylgowa, OdsunięcieInoxMałyWewWzględemOsiYStronaPrzylgowa), PraweIsTrue, nazwaModelu: NazwaModelu);
        InoxMałyGeometriaStronaFelcowa = new(new Prostokąt(InoxMałyZewSzerokośćStronaFelcowa, InoxMałyZewWysokośćStronaFelcowa, !PraweIsTrue, InoxMałyZewZaokrąglenieStronaFelcowa), new Prostokąt(InoxMałyWewSzerokośćStronaFelcowa, InoxMałyWewWysokośćStronaFelcowa, !PraweIsTrue, InoxMałyWewZaokrąglenieStronaFelcowa, OdsunięcieInoxMałyWewWzględemOsiXStronaFelcowa, OdsunięcieInoxMałyWewWzględemOsiYStronaFelcowa), !PraweIsTrue, nazwaModelu: NazwaModelu);
        InoxTekstura = new(InoxKolor, InoxZewSzerokośćStronaPrzylgowa, InoxZewWysokośćStronaPrzylgowa, SkalaParametr);
        InoxMałyTekstura = new(InoxKolor, InoxMałyZewSzerokośćStronaPrzylgowa, InoxMałyZewWysokośćStronaPrzylgowa, SkalaParametr);
    ...
    protected override void RysujInox()
    {
        ...
        for (int i = 0; i < paths.Length; i++)
        {
            ...
            pathsMały[i] = new()
            {
                Data = InoxMałyGeometriaStronaPrzylgowa.KombinacjaFigur,
                StrokeThickness = 0,
                Fill = InoxMałyTekstura.TeksturaObcięta,
                Stretch = Stretch.Fill,
                Width = InoxMałyZewSzerokośćStronaPrzylgowa * SkalaParametr,
                Height = InoxMałyZewWysokośćStronaPrzylgowa * SkalaParametr,
                VerticalAlignment = VerticalAlignment.Bottom,
                HorizontalAlignment = HorizontalAlignment.Left,
                Margin = new Thickness((odsunięcieInoxOdKrawędziX - OdległośćPomiędzyInox - InoxMałyZewSzerokośćStronaPrzylgowa) * SkalaParametr, 0d, 0d, (i * odsunięcieInoxOdKrawędziY + 300d - InoxZewWysokośćStronaPrzylgowa / 2) * SkalaParametr)
            };
            ...
            if (ZewIsTrue == false)
            {
                ...
                pathsMały[i].Data = InoxMałyGeometriaStronaFelcowa.KombinacjaFigur;
                pathsMały[i].Width = InoxMałyZewSzerokośćStronaFelcowa * SkalaParametr;
            }
            ...
            //pathsMały[i].Stroke = Brushes.Black;
            //pathsMały[i].StrokeThickness = 1;
            //pathsMały[i].Fill = Brushes.Transparent;

            grid.Children.Add(paths[i]);
            grid.Children.Add(pathsMały[i]);
        }
    ...
    public override void GenerujDXF()
    {
        ...
        InoxMałyGeometriaStronaPrzylgowa.ZapiszPlikDXF();
    }
}

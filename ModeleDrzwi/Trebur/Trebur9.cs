...
public class Trebur9 : Drzwi
{
    ...
    public Trebur9(Ościeżnica ościeżnica, Skrzydło skrzydło, SzyldDolny szyldDolny, Klamka klamka, SzyldGórny szyldGórny,
    ...
    protected void GeometriaModelu()
    {
        ...
        PasekInoxSzerokość = Skrzydło.SzerokośćWPrzyldze - (260d + InoxZewSzerokośćStronaPrzylgowa / 2 + 1 * PasekInoxOdsunięcie) - (260d - InoxZewSzerokośćStronaPrzylgowa / 2);
    ...
    protected override void RysujInox()
    {
        Path pathInox;
        Grid grid = new();
        double odsunięcieInoxOdKrawędziX = Skrzydło.SzerokośćWPrzyldze - (260d + InoxZewSzerokośćStronaPrzylgowa / 2d);
        double odsunięcieInoxOdKrawędziY = (Skrzydło.WysokośćWPrzyldze - InoxZewWysokośćStronaPrzylgowa) / 2d;

        Path[] pathPaskiInox = new Path[5];
        ...
        double odległośćMiędzyPaskamiInox = (InoxZewWysokośćStronaPrzylgowa - 1 * PasekInoxWysokość) / 4;
        ...
        for (int i = 0; i < pathPaskiInox.Length; i++)
        {
            pathPaskiInox[i] = new()
            {
                ...
                VerticalAlignment = VerticalAlignment.Bottom,
                HorizontalAlignment = HorizontalAlignment.Left,
                Margin = new Thickness(odsunięciePaskiInoxOdKrawędziX * SkalaParametr, 0d, 0d, (odsunięciePaskiInoxOdKrawędziY + i * odległośćMiędzyPaskamiInox) * SkalaParametr)
            };
    ...
    protected override void RysujSzkło()
    {
        Path paths;
        Grid grid = new();
        double odsunięcieSzkłoOdKrawędziX = Skrzydło.SzerokośćWPrzyldze - (260d + SzkłoSzerokość / 2d);

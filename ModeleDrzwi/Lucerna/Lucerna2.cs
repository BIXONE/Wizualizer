...
public class Lucerna2 : Drzwi
{
    ...
    public Lucerna2(Ościeżnica ościeżnica, Skrzydło skrzydło, SzyldDolny szyldDolny, Klamka klamka, SzyldGórny szyldGórny,
    ...
    protected override void RysujSzkło()
    {
        Path[] paths = new Path[4];
        Grid grid = new();
        double odsunięcieSzybyOdKrawędziX = (Skrzydło.SzerokośćWPrzyldze - SzkłoSzerokość) / 2d;
        ...
        for (int i = 0; i < paths.Length; i++)
        {
            paths[i] = new()
            {
                ...
                VerticalAlignment = VerticalAlignment.Bottom,
                HorizontalAlignment = HorizontalAlignment.Left,
                Margin = new Thickness(odsunięcieSzybyOdKrawędziX * SkalaParametr, 0d, 0d, (i * odsunięcieSzkłoOdKrawędziY + 300 - SzkłoWysokość / 2) * SkalaParametr)
            };
